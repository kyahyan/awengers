...
export class HeroAltarModal {
    private modal: ModalWrapper;
    private user: UserProfile;
    private contentArea!: HTMLElement;
    private currentTab: AltarTab = 'decompose';
    private selectedHeroIds: string[] = []; // Multi-select for decompose (up to 14)
    private onUpdate: (updatedUser: any) => void;
    private onClose: () => void;

    private readonly MAX_DECOMPOSE_SELECT = 14;

    constructor(user: UserProfile, onUpdate: (updatedUser: any) => void, onClose: () => void) {
        this.user = user;
    ...
    private initialize() {
        const container = this.modal.getContentArea();
        container.innerHTML = '';
        ...
        const createTab = (id: AltarTab, label: string, icon: string) => {
            ...
            tab.style.cssText = `
                ...
                border-bottom: ${isActive ? '3px solid #fbbf24' : '3px solid transparent'};
            `;
            tab.onmouseenter = () => {
                if (!isActive) {
    ...
    private render() {
        ...
        // Show selection count for decompose
        const selectionInfo = this.currentTab === 'decompose' && this.selectedHeroIds.length > 0
            ? `<div style="color: #fbbf24; font-size: 0.9rem; margin-top: 8px;">📦 Selected: ${this.selectedHeroIds.length}/${this.MAX_DECOMPOSE_SELECT}</div>`
            : '';

        descBox.innerHTML = `
            <div style="font-size: 1.2rem; font-weight: bold; color: #fbbf24; margin-bottom: 8px;">${info.title}</div>
            <div style="color: #9ca3af; font-size: 0.9rem; margin-bottom: 8px;">${info.desc}</div>
        ...
        gridLabel.style.cssText = 'color: #9ca3af; font-size: 0.85rem; margin-bottom: 10px;';
        ...
        heroEntries.forEach(([instanceId, heroData]: [string, any]) => {
            ...
            // Filter based on tab requirements
            ...
            card.style.cssText = `
                position: relative;
                background: ${isSelected ? 'rgba(251, 191, 36, 0.2)' : 'rgba(0, 0, 0, 0.4)'};
                border: 2px solid ${isSelected ? '#fbbf24' : 'rgba(255, 255, 255, 0.1)'};
                border-radius: 10px;
                padding: 8px;
            ...
            // Derive portrait path from sprite2D spritesheetPath
            let portraitPath = '';
            ...
            starsDiv.innerHTML = Array(heroStars).fill('<span style="color: #fbbf24;">★</span>').join('');
            card.appendChild(starsDiv);

            // Name
            const nameDiv = document.createElement('div');
            nameDiv.style.cssText = 'font-size: 0.7rem; color: #fff; text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;';
            nameDiv.textContent = heroCodeName;
            card.appendChild(nameDiv);

            // Level and Attribute
            const levelDiv = document.createElement('div');
            levelDiv.style.cssText = 'font-size: 0.65rem; color: #9ca3af; text-align: center;';
            ...
            if (!isDeployed) {
                card.onmouseenter = () => {
                    if (!isSelected) card.style.borderColor = 'rgba(251, 191, 36, 0.5)';
                };
                card.onmouseleave = () => {
                    if (!isSelected) card.style.borderColor = 'rgba(255, 255, 255, 0.1)';
                };
                card.onclick = () => {
                    if (this.currentTab === 'decompose') {
                        // Multi-select for decompose
                        ...
                        // Single select for reset/star-back
        ...
        if (this.currentTab === 'decompose' && this.selectedHeroIds.length > 0) {
            ...
            actionBox.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                    <div style="font-size: 1.1rem; font-weight: bold; color: #ef4444;">💀 Decompose ${this.selectedHeroIds.length} Hero${this.selectedHeroIds.length > 1 ? 'es' : ''}</div>
                    <button id="clear-selection-btn" style="background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.2); color: #9ca3af; padding: 5px 10px; border-radius: 5px; cursor: pointer; font-size: 0.8rem;">Clear All</button>
                </div>
            ...
            const borderColor = this.currentTab === 'reset' ? 'rgba(37, 99, 235, 0.5)' : 'rgba(245, 158, 11, 0.5)';
                ...
                rewardsHtml = `
                    <div style="display: flex; gap: 15px; margin-bottom: 12px;">
                        <div style="display: flex; align-items: center; gap: 6px;">
                            ...
                            <span style="color: #fbbf24; font-weight: bold;">+${goldReturn.toLocaleString()}</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 6px;">
                            <img src="/assets/potions/soul-potion-icon.png" style="width: 28px; height: 28px;">
            ...
            const label = this.currentTab === 'reset' ? 'Reset Level' : 'Star-Back';
            ...
            actionBox.innerHTML = `
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
                    ...
                    <div>
                        <div style="font-size: 1rem; font-weight: bold; color: #fff;">${heroCodeName}</div>
                        <div style="font-size: 0.85rem; color: #9ca3af;">${'★'.repeat(heroStars)} • Level ${heroLevel} • ${heroAttr}</div>
                    </div>
                </div>
                ${rewardsHtml}
            `;
    ...
    private async executeAction() {
        ...
        if (this.currentTab === 'decompose') {
            ...
            // Single hero action (reset/star-back)
